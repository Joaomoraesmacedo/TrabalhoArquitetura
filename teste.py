...
def  executa_instrucao(pipeline, valor_reg) -> tuple[int, int]:
...
def verifica_fim(pipeline, pc, lista_inst) -> bool:
    comando = True
    verifica_noop = True 
    for i in pipeline:
        if i != None:
...
def executa_operacoes(operacao:io.TextIOWrapper, nome_reg:list[str], valor_reg:list[int], memoria:list[int]):
    ...
    while comando:
        ...
        #Faz a execucao de instrucoes 
        if pipeline[2] is not None:
            reg, valor = executa_instrucao(pipeline, valor_reg)
            resultados.append((reg,valor))
        ...
        #Verifica se todos os estagios do pipeline estão vazios 
        comando = verifica_fim(pipeline, pc, lista_inst)
        if pipeline[4] is not None:
            if resultados:
                resultado = resultados.pop(0)
                if len(resultado) == 2:
                    registrador, valor = resultado
                    valor_reg[registrador] = valor
                else:
                    print("Erro: tupla de resultado incompleta!", resultado)
            else:
                print("Erro: nenhum resultado disponível para escrever no registrador.")
